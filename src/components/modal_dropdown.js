...
export default class ModalDropdown extends Component {
  ...
  _renderButton(fullBorder = true) {
    const {disabled, accessible, children, textStyle} = this.props;
    const {buttonText, showDropdown} = this.state;

    const borderBottomRightRadius = fullBorder ? size(10) : 0;
    const borderBottomLeftRadius = fullBorder ? size(10) : 0;
    ...
    return (
      <TouchableOpacity ref={button => this._button = button}
        ...
        style={{
            height: size(59), width: WIDTH * 0.88,backgroundColor: 'white',alignSelf: 'center',justifyContent: 'center',borderRadius: size(10),
        ...
        >
          ...
          >
            <View style={styles.modal}>
              <View style={{position: 'absolute',top: frameStyle.top - size(59),zIndex: -1,alignSelf: 'center'}}>
                {this._renderButton(false)}
              </View>
              <View style={[styles.dropdown, dropdownStyle, frameStyle, { backgroundColor: 'white' }]}>
  ...
  _renderRow = (rowData, sectionID, rowID, highlightRow) => {
    ...
    return (
      <TouchableOpacity activeOpacity={ 1 } style={{height: size(65),justifyContent: 'center',}} {...preservedProps}>
        {row}
      </TouchableOpacity>
    );
  };
